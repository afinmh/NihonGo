...
import { natoriConfig } from './model/natori';
...
console.log('[Chapter5] Initializing Live2D scene (Natori)...');
...
function ensureLayoutDefaults(cfg: ModelConfig) {
    ...
    return {
        xFrac: layout.xFrac ?? 0.25,
        yFrac: layout.yFrac ?? 0.5,
        anchorX: layout.anchorX ?? 0.5,
        anchorY: layout.anchorY ?? 0.25,
        targetWidthFrac: layout.targetWidthFrac ?? 0.9,
        targetHeightFrac: layout.targetHeightFrac ?? 2.3,
    };
}
...
function bindGlobalAudioHelpers(motionGroups: Record<string, string> | undefined) {
    ...
    const alt = motionGroups?.disagree || 'Flick@Body';
    const happy = motionGroups?.cheer || 'FlickUp@Head';
...
async function init() {
    const cfg = natoriConfig; // gunakan Natori untuk Chapter 5
    ...
    try {
        ...
        console.log('[Chapter5] Natori model loaded and added to stage.');
        // @ts-ignore
        (window as any).live2dModel = model;
    } catch (err) {
        console.error('[Chapter5] Failed to load Natori model:', err);
        return;
    } finally {
        try {
    ...
    const mobileLayout = {
        ...
        anchorX: 0.5,
        anchorY: 0.45,
        targetWidthFrac: 1.1,   // Model mengisi 100% lebar layar
        targetHeightFrac: 1.5,  // Model mengisi 100% tinggi layar
    };
