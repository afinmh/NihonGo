...
function playAudioHappy(src) {
    ...
    // Materi Chapter 3
    const materiList = [
        { id: 1, key: 'sekolah', title: 'Kosakata tentang Sekolah', jsonUrl: '/scripts/chapter3/sekolah.json', rootKey: 'kosakata_sekolah', audioBase: 'sekolah' },
        { id: 2, key: 'perlengkapan', title: 'Benda-benda di Sekolah', jsonUrl: '/scripts/chapter3/perlengkapan.json', rootKey: 'benda_sekolah', audioBase: 'perlengkapan' },
        { id: 3, key: 'teman', title: 'Teman', jsonUrl: '/scripts/chapter3/teman.json', rootKey: 'teman', audioBase: 'teman' },
        { id: 4, key: 'aktivitas', title: 'Aktivitas di Sekolah', jsonUrl: '/scripts/chapter3/aktivitas.json', rootKey: 'aktivitas_sekolah', audioBase: 'aktivitas' },
        { id: 5, key: 'kalimat', title: 'Kalimat Sederhana', jsonUrl: '/scripts/chapter3/kalimat.json', rootKey: 'kalimat_sederhana_sekolah', audioBase: 'kalimat' },
    ];
    ...
    // Load saved state from localStorage (Chapter 3)
    try {
        const saved = JSON.parse(localStorage.getItem('chap3Progress') || 'null');
    ...
    try { localStorage.setItem('chap3Progress', JSON.stringify(payload)); } catch {}
    }
    ...
    if (percent >= 80) {
            ...
            setTimeout(() => {
                addMessage('Selamat!', 'reply');
                    playAudio('/audio/chapter3/hebat.mp3');
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        if (percent === 100) {
                            addMessage('Hebat!', 'reply');
                                playAudioHappy('/audio/chapter3/hebat.mp3');
                        }
            ...
            setTimeout(() => {
                addMessage('Belum lulus. Coba lagi ya!', 'reply');
                    playAudio('/audio/chapter3/salah.mp3');
                // Kembali ke menu materi
                setTimeout(() => {
                    clearChat();
                    addMessage('Pilih materi:', 'reply');
        ...
        if (allComplete && !already) {
            ...
            try {
                if (typeof playAudioHappy === 'function') {
                    playAudioHappy('/audio/chapter3/finish.mp3');
                } else {
                    playAudio('/audio/chapter3/finish.mp3');
                }
            } catch {}
        }
    }
    ...
    if (formEl) {
        formEl.addEventListener('submit', (e) => {
            ...
            if (correct) {
                ...
                playAudioHappy('/audio/chapter3/benar.mp3');
            } else {
...
function startChapter1() {
    ...
    addMessage('Oke, kenalin aku Akira!', 'reply');
    playAudioHappy('/audio/chapter3/intro.mp3');

    // jeda 2 detik
    setTimeout(() => {
        addMessage('Senang berkenalan denganmu!', 'reply');
    playAudio('/audio/chapter3/post-intro.mp3');

        // jeda 2 detik
        setTimeout(() => {
            addMessage('Aku bakal temenin kamu belajar di sekolah.', 'reply');
            playAudio2('/audio/chapter3/learn.mp3');

            // jeda 1.5 detik
            setTimeout(() => {
                addMessage('Jadi, Ayo kita mulai!', 'reply');
                playAudioHappy('/audio/chapter3/lets-start.mp3');
...
function startRumah() {
    return startMateriCommon({
        materiIndex: 0,
        title: 'Kosakata tentang Sekolah',
        jsonUrl: '/scripts/chapter3/sekolah.json',
        rootKey: 'kosakata_sekolah',
...
function startPerlengkapan() {
    return startMateriCommon({
        materiIndex: 1,
        title: 'Benda-benda di Sekolah',
        jsonUrl: '/scripts/chapter3/perlengkapan.json',
        rootKey: 'benda_sekolah',
...
function startKeluarga() {
    return startMateriCommon({
        materiIndex: 2,
        title: 'Teman',
        jsonUrl: '/scripts/chapter3/teman.json',
        rootKey: 'teman',
...
function startAktivitas() {
    return startMateriCommon({
        materiIndex: 3,
        title: 'Aktivitas di Sekolah',
        jsonUrl: '/scripts/chapter3/aktivitas.json',
        rootKey: 'aktivitas_sekolah',
...
function startKalimat() {
    return startMateriCommon({
        ...
        jsonUrl: '/scripts/chapter3/kalimat.json',
        rootKey: 'kalimat_sederhana_sekolah',
    ...
    if (hour >=5 && hour <11) { greetingText='Selamat pagi!'; greetingAudio='/audio/chapter3/pagi.mp3'; }
    else if (hour >=11 && hour <15) { greetingText='Selamat siang!'; greetingAudio='/audio/chapter3/siang.mp3'; }
    else if (hour >=15 && hour <18) { greetingText='Selamat sore!'; greetingAudio='/audio/chapter3/siang.mp3'; }
    else { greetingText='Selamat malam!'; greetingAudio='/audio/chapter3/malam.mp3'; }

        addMessage(greetingText, 'reply');
        playAudio(greetingAudio);

        setTimeout(() => {
            addMessage('Di Chapter 3 kita akan belajar tentang sekolah. Kamu siap?', 'reply');
            playAudio2('/audio/chapter3/ready.mp3');
            showReadyButtons();
        }, 2500);
    }
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            const yakin = confirm('Yakin reset semua progres Chapter 3?');
            if (!yakin) return;
            try { localStorage.removeItem('chap3Progress'); } catch {}
            // reset in-memory
